...
// 向链表中插入结点实现
int insert_node(list_t* p_list, airplane_t const* p_air) {
	...
	p_node->air = *p_air;  //	将p_node所指向的结点的数据域进行赋值 
	// 如果要插入的位置是第1个结点位置处，此处有两种情况 
	// 1. p_list->head 为空 
	// 2. 将要插入的结点的id小于等于头结点的id   插入结点时  id小的放在前面 id大的在后

	if ((!p_list->head) || (p_node->air.id <= p_list->head->air.id)) {
	...
	for (; p_curr; p_curr = p_curr->p_next) {
		...
		// 则新将要将插入的结点插入到当结点之前 
		if (p_curr->air.id >= p_node->air.id) {
			// 新结点的指针域赋值为当前结点地址
